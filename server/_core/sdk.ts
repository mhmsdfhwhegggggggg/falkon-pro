...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // Check user cache
    const cachedUser = this.userCache.get(sessionCookie ?? "");
    const now = Date.now();
    if (cachedUser && (now - cachedUser.timestamp) < this.USER_CACHE_TTL) {
    ...
    // Skip redundant updates on every request to prevent DB contention
    /*
    if (user) {
      await db.upsertUser({
        email: user.email,
